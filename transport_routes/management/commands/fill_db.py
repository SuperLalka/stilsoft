...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if route_objects < 200:
            while route_objects < 200:
                ...
                if status:
                    for num in range(5):
                        transport = Transport.objects.create(board_number=random.randint(111111, 999999),
                                                             transport_type=route.route_type)
                        transport.save()
                        route.transports.add(transport)

                        print(datetime.now(), 'transport {0} / {1} for route created'.format(transport.board_number,
